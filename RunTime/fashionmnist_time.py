...
output_filename = "fashionmnist_runtimes.txt"
...
with open(output_filename, 'w') as f:
    ...
    log("="*60)
    log(f"Starting Fashion-MNIST Runtime Benchmark")
    log(f"Date: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    log(f"Results will be saved to: {output_filename}")
    log("="*60)
    log(f"Dataset: Fashion-MNIST")
    log(f"Rates to test: {rates_to_test}")
    log(f"Runs per rate: {n_runs}")
    log("="*60)

    # Load Fashion-MNIST labels once
    log("\nLoading Fashion-MNIST labels...")
    ...
    W = gl.weightmatrix.knn('fashionmnist', 10, metric='vae', kernel='gaussian')
    ...
    for rate in rates_to_test:
        ...
        for run in range(n_runs):
            ...
            if (run + 1) % 5 == 0:
                print(f"  Running trial {run + 1}/{n_runs}...") # Only print progress to console
