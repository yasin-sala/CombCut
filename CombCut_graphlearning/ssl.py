...
class poisson(ssl):
    def __init__(self, W=None, class_priors=None, solver='conjugate_gradient', p=1, use_cuda=False, min_iter=1, max_iter=1, tol=1e-3, spectral_cutoff=10):
        """Poisson Learning
        ===================
...
class poisson_mbo(ssl):
    def __init__(self, W=None, class_priors=None, solver='conjugate_gradient', use_cuda=False, min_iter=50, max_iter=1, tol=1e-3, spectral_cutoff=10, Ns=1, mu=1, T=1):
        """PoissonMBO 
        ===================
...
class multiclass_mbo(ssl):
    def __init__(self, W=None, class_priors=None, Ns=1, T=1, dt=0.15, mu=50, num_eig=50):
        """Multiclass MBO
        ===================
...
class modularity_mbo(ssl):
    def __init__(self, W=None, class_priors=None, gamma=0.5, epsilon=1, lamb=1, T=1, Ns=1):
        """Modularity MBO
        ===================
...
class laplace(ssl):
    def __init__(self, W=None, class_priors=None, X=None, reweighting='none', normalization='combinatorial', 
                 tau=0, order=1, mean_shift=False, tol=100000, alpha=2, zeta=1e7, r=0.1):
        """Laplace Learning
        ===================
        ...
        [5] Holtz, Chester, et al. "Continuous Partitioning for Graph-Based Semi-Supervised Learning." The Thirty-eighth Annual Conference on Neural Information Processing Systems. 2024.
        """
        super().__init__(W, class_priors)
...
class dynamic_label_propagation(ssl):
    def __init__(self, W=None, class_priors=None, alpha=0.05, lam=0.1, T=1):
...
class centered_kernel(ssl):
    def __init__(self, W=None, class_priors=None, tol=1000000000, power_it=100, alpha=1.05):
...
class sparse_label_propagation(ssl):
    def __init__(self, W=None, class_priors=None, T=1):
...
class amle(ssl):
    def __init__(self, W=None, class_priors=None, tol=1e7, max_num_it=1, weighted=False, prog=False):
        """AMLE learning
        ===================
...
class peikonal(ssl):
    def __init__(self, W=None, class_priors=None, D=None, p=1, alpha=1, max_num_it=1, tol=1e-3, num_bisection_it=30, eps_ball_graph=False):
...
class plaplace(ssl):
    def __init__(self, W=None, class_priors=None, p=10, max_num_it=1, tol=1000, fast=True):
...
class randomwalk(ssl):
    ...
    def _fit(self, train_ind, train_labels, all_labels=None):
        ...
        return u
# -----------------------------------------------------------------------------
# Continuous Partitioning (CutSSL) — Holtz et al. NeurIPS 2024
# -----------------------------------------------------------------------------
class cut_ssl(ssl):
    def __init__(self, W=None, class_priors=None, s=[0.2], maxiter=1):
        """CutSSL
        ===================
        ...
        self.name = 'CutSSL'
        self.accuracy_filename = '_cutssl'
        self.k = None  # Will be set in _fit

    def _conjgradfn(self, A, b, x0=None, max_iter=100, tol=1e-10):
    ...
    def mu1(self, b, y, lamb, mu2, c, o, m, I2Aio):
        """Compute mu1 multiplier"""
        on, ok = o
        mu = (1/c) * ((y + b - lamb - mu2 @ ok.T).T @ I2Aio - m)
        return mu

    def mu2(self, b, y, lamb, mu1, o, Aon):
        """Compute mu2 multiplier"""
        on, ok = o
        return (1/self.k) * ((y + b - lamb - on @ mu1.T) @ ok - Aon)

    def yklambk(self, xk, lambk):
        """Update yk and lambk variables"""
        yk = np.maximum((lambk + xk), 0.)
        lambk = lambk + (xk - yk)
        return yk, lambk

    def Ainv(self, A, b):
        """Inverse operation using preconditioned conjugate gradient"""
        b = np.array(b)
        # Preconditioner
        m = A.shape[0]
        M = A.diagonal()
        ...
        # Conjugate gradient solver
        v = utils.conjgrad(M*A*M, M*b, tol=1e-5, max_iter=200)
        xs = M*v
        return xs

    def _fit(self, train_ind, train_labels, all_labels=None):
        """Main fitting routine"""
        G = graph.graph(self.graph.weight_matrix)
        D = G.degree_matrix()
        ...
        M = n - len(train_ind)
        unique_labels = np.unique(train_labels)
        self.k = len(unique_labels)

        # Graph Laplacian and one-hot labels
        L = G.laplacian(normalization='combinatorial')
        F = utils.labels_to_onehot(train_labels, self.k)

        # Compute C and its square root
        p = n / self.k
        r = -F.sum(axis=0, keepdims=True).T
        C = p * np.eye(self.k) - (F.T @ F) - (1.0/n) * (r @ r.T)
        
        # Matrix square root
        valsC, vecsC = np.linalg.eigh(C)
        sqrtC = vecsC @ np.diag(np.sqrt(np.maximum(valsC, 0))) @ vecsC.T
        ...
        # Initial conjugate gradient solve
        _P = lambda x : x - 1/n * on @ (on.T @ x)
        _A = lambda x : _P(A0 @ (_P(x)))
        _xk = self._conjgradfn(_A, _P(b), max_iter=100, tol=1e-6)
        _xk = _xk + _xp
        ...
        for j, _s in enumerate(self.s):
            ...
            for i in range(self.maxiter):
                ...
                yk, lambk = self.yklambk(_xk, lambk)
                ...
                if (all_labels is not None) and (i%25 == 0):
                    acc = gl.ssl.ssl_accuracy(u.argmax(1), all_labels, train_ind)
...
class SSM_SSL(ssl):
    """
    Sequential Subspace Method for Semi-Supervised Laplace Learning on the Stiefel manifold,
    with C = I and exact mean-shift restoration.
    """
    def __init__(self, W=None, class_priors=None, max_iter=1, tol=1e-6, subspace_dim=4):
        super().__init__(W, class_priors)
        self.max_iter     = max_iter     # maximum SSM iterations
        self.tol          = tol          # convergence tolerance
        self.subspace_dim = subspace_dim # #eigenvectors per class for subspace
        self.name         = 'SSM_SSL'
        self.requires_eig = True

    def _stiefel_proj(self, M):
        """Project M → argmin_{X^T X=I} ||X−M||_F via SVD (Eq.8)."""
        U, _, Vt = np.linalg.svd(M, full_matrices=False)
        return U @ Vt

    def _build_LB(self, train_ind, train_labels):
        # 1) Partition nodes into labeled/unlabeled
        n = self.graph.num_nodes
        ...
        # 2) One-hot label matrix F
        classes = np.unique(train_labels)
        k = len(classes)
        F = np.zeros((len(train_ind), k))
        F[np.arange(len(train_ind)), train_labels] = 1

        # 3) Combinatorial Laplacian L = D − W
        W_mat = self.graph.weight_matrix
        D     = diags(W_mat.sum(axis=1).A.ravel())
        L     = D - W_mat

        # 4) Partition L_uu and L_ul
        L_uu = L[unl[:, None], unl].tocsr()
        L_ul = L[unl[:, None], train_ind].tocsr()

        # 5) Build B = L_ul @ F   (C = I)
        B = L_ul.dot(F)

        return L_uu, B, unl, F, k

    def _init_X(self, L, B, k):
        # Eigeninit: smallest k eigenvectors of L, then Procrustes to B (Eqs.12–13)
    ...
    def _fit(self, train_ind, train_labels, all_labels=None):
        ...
        # Assemble L, B, unlabeled indices, and one-hots
        L, B, unl, F, k = self._build_LB(train_ind, train_labels)
        n = self.graph.num_nodes
        m = len(unl)

        # Compute shift vector r = –sum_cols(F) to restore global centering (Eq.7)
        r = -F.sum(axis=0, keepdims=True).T  # shape (k,1)

        # 1) Initialization on Stiefel
        X = self._init_X(L, B, k)

        # 2) Precompute extra eigenvectors for subspace (Alg.2)
        _, extra_eigs = eigsh(L, k=self.subspace_dim * k, which='SM')

        # 3) Main SSM loop
        for it in range(self.max_iter):
            # a) Gradient G = L X − B  (Eq.14)
            G = L.dot(X) - B

            # b) SQP directions Z via tangent-space solves (Eq.15)
            Lam, Ue = np.linalg.eigh(X.T @ G)
            P = np.eye(m) - X @ X.T
            Z = np.zeros((m, k))
            for j in range(k):
                A   = P.dot(L.dot(P)) - Lam[j] * P
                rhs = P.dot(B.dot(Ue[:, j:j+1]))
                z   = spsolve(A, rhs)
                Z[:, j] = z.ravel()

            # c) Build subspace S = [X, G, Z, extra_eigs[:, :k]] (Alg.2)
            S = np.concatenate([X, G, Z, extra_eigs[:, :k]], axis=1)
            Q, _ = np.linalg.qr(S, mode='reduced')

            # d) Restrict: Ls = Q^T L Q, Bs = Q^T B; re-solve via Procrustes
            Ls = Q.T @ L.dot(Q)
            Bs = Q.T @ B
            Y0 = self._init_X(Ls, Bs, k)

            # e) Lift + Stiefel-project (Eq.16 & Eq.8)
            X_new = self._stiefel_proj(Q.dot(Y0))
        ...
        # 4) Restore full solution u with mean-shift:
        #    X_u = X + (1/n)·1·r^T  and then stack labeled F
        X_u = X + np.ones((m, 1)) @ (r.T) / n
        u   = np.zeros((n, k))
        u[unl]       = X_u
        u[train_ind] = F
        return u
...
# ----------------------------------------------------------------------
# Projection‐Based MM‐Inspired Exact CutSSL (CombCutSSL)
# ----------------------------------------------------------------------
...
class CombCutSSL(ssl):
    def __init__(self, W=None, class_priors=None, max_iter=1, tol=1e-3,
                 step_size=0.1, sinkhorn_max_iter=1, sinkhorn_tol=1e-3,
                 use_largest_eigenvalue=True, s_shift=None, init_method='cutssl'):
        """CombCutSSL - Combinatorial Cut SSL following exact mathematical approach
        ===================

        Semi-supervised learning via combinatorial solutions to a cardinality-constrained min-cut problem,
        following the MM framework with projection-based optimization.
        ...
        step_size : float or callable (optional), default=0.1
            Gradient step size for projection method.
        sinkhorn_max_iter : int (optional), default=1000
            Maximum iterations for Sinkhorn projection.
        sinkhorn_tol : float (optional), default=1e-3
            Tolerance for Sinkhorn projection.
        use_largest_eigenvalue : bool (optional), default=True
            Whether to compute s as largest eigenvalue of L_uu.
        s_shift : float (optional), default=None
            Manual shift value (overrides eigenvalue computation).
        init_method : str (optional), default='cutssl'
            Initialization method: 'cutssl', 'random', 'spectral', or 'uniform'.
        ...
        self.max_iter = max_iter
        self.tol = tol
        self.step_size = step_size if callable(step_size) else lambda t: step_size
        self.sinkhorn_max_iter = sinkhorn_max_iter
        self.sinkhorn_tol = sinkhorn_tol
        self.use_largest_eigenvalue = use_largest_eigenvalue
        self.s_shift = s_shift
        self.init_method = init_method
        
        self.name = 'CombCutSSL (Exact Method)'
        self.accuracy_filename = '_combcutssl_exact'
        self.requires_eig = use_largest_eigenvalue

    #def _solve_surrogate_projection(self, C, m_u, X_current):
    #    n, k = C.shape
    #    col_labels = np.concatenate([np.full(m_u[j], j, dtype=int) for j in range(k)])
    #    C_expanded = C[:, col_labels]
    ...
    def _project_onto_doubly_stochastic(self, X, m_u, max_iter=None, tol=None):
        """Project X onto the constraint set C using alternating projections"""
        if max_iter is None:
        ...
        n, k = X.shape
        X_proj = np.clip(X, 0, 1)  # Ensure [0,1] bounds
        
        for _ in range(max_iter):
            X_old = X_proj.copy()
            
            # Project onto row constraints: X @ 1_k = 1_n
            row_sums = X_proj.sum(axis=1, keepdims=True)
            row_sums[row_sums == 0] = 1  # Avoid division by zero
            X_proj = X_proj / row_sums
            
            # Project onto column constraints: X^T @ 1_n = m_u
            col_sums = X_proj.sum(axis=0)
            if col_sums.sum() > 0:
                X_proj = X_proj * (m_u / col_sums)
            
            # Ensure bounds [0,1]
            X_proj = np.clip(X_proj, 0, 1)
            
            # Check convergence
            if np.linalg.norm(X_proj - X_old) < tol:
                break
                
        return X_proj

    def _solve_surrogate_projection(self, C, m_u, X_current):
        """Solve surrogate using projection-based first-order method"""
        n, k = C.shape
        eta = self.step_size(0)  # Step size
        
        # Gradient step
        X_half = X_current + eta * C
        
        # Project onto constraint set
        X_new = self._project_onto_doubly_stochastic(X_half, m_u)
        
        return X_new

    def _initialize_unlabeled_assignments(self, n_unlabeled, k, m_u, L_uu, B, train_ind, train_labels):
        """Initialize X_u for unlabeled nodes using different methods"""
        
        if self.init_method == 'cutssl':
            # Original CutSSL initialization
            try:
                cutssl_model = cut_ssl(W=self.graph.weight_matrix, class_priors=self.class_priors, s=[0.2], maxiter=1)
                X_initial = cutssl_model.fit(train_ind, train_labels, all_labels=None)
                unlabeled_mask = np.ones(self.graph.num_nodes, dtype=bool)
                unlabeled_mask[train_ind] = False
                X_u = X_initial[unlabeled_mask]
            except:
                print("CutSSL initialization failed, using spectral initialization")
                X_u = self._spectral_initialization(n_unlabeled, k, m_u, L_uu, B)
                
        elif self.init_method == 'spectral':
            # Spectral initialization using eigenvectors of L_uu
            X_u = self._spectral_initialization(n_unlabeled, k, m_u, L_uu, B)
            
        elif self.init_method == 'random':
            # Random initialization
            X_u = np.random.rand(n_unlabeled, k)
            
        elif self.init_method == 'uniform':
            # Uniform initialization
            X_u = np.ones((n_unlabeled, k)) / k
            
        else:
            raise ValueError(f"Unknown initialization method: {self.init_method}")
        
        # Always ensure constraints are satisfied
        X_u = self._project_onto_doubly_stochastic(X_u, m_u)
        
        return X_u
    
    def _spectral_initialization(self, n_unlabeled, k, m_u, L_uu, B):
        """Spectral initialization using smallest eigenvectors of L_uu"""
        try:
            # Get k smallest eigenvectors of L_uu
            if n_unlabeled > k and sparse.issparse(L_uu):
                eigenvals, eigenvecs = eigsh(L_uu, k=k, which='SM', sigma=0.0)
            else:
                # For small matrices, use dense computation
                L_dense = L_uu.toarray() if sparse.issparse(L_uu) else L_uu
                eigenvals, eigenvecs = np.linalg.eigh(L_dense)
                eigenvecs = eigenvecs[:, :k]
            
            # Use eigenvectors as initialization
            X_u = np.abs(eigenvecs)  # Ensure non-negative
            
            # Add small random perturbation to break symmetry
            X_u += 0.01 * np.random.rand(n_unlabeled, k)
            
        except:
            print("Spectral initialization failed, using uniform initialization")
            X_u = np.ones((n_unlabeled, k)) / k
            
        return X_u

    def _compute_objective(self, X, M, B):
        """Compute F(X) = 0.5 * tr(X^T @ M @ X) + tr(X^T @ B)"""
        return 0.5 * np.trace(X.T @ M @ X) + np.trace(X.T @ B)

    def _fit(self, train_ind, train_labels, all_labels=None):
        """Internal fit function following the exact mathematical approach"""
        ...
        # One-hot labels for labeled nodes
        Y = utils.labels_to_onehot(train_labels, k)  # m x k
        
        # Create unlabeled mask
        ...
        L = G.laplacian(normalization="combinatorial")
        L_uu = L[unlabeled_mask][:, unlabeled_mask]  # n_unlabeled x n_unlabeled
        L_ul = L[unlabeled_mask][:, train_ind]       # n_unlabeled x m
        
        # Compute supervision term B = L_ul @ Y
        B = L_ul @ Y  # n_unlabeled x k
        
        # Compute class sizes for unlabeled nodes
        if self.class_priors is not None:
            m_u = np.round(self.class_priors * n_unlabeled).astype(int)
        else:
            # Default: distribute remaining nodes proportionally
            labeled_counts = Y.sum(axis=0)
            total_per_class = np.round((n / m) * labeled_counts).astype(int)
            m_u = np.maximum(total_per_class - labeled_counts, 1)
        
        # Ensure m_u sums to n_unlabeled
        m_u = m_u * n_unlabeled // m_u.sum()
        if m_u.sum() != n_unlabeled:
            m_u[0] += n_unlabeled - m_u.sum()
        
        # Compute shift parameter s >= λ_max(L_uu)
        if self.use_largest_eigenvalue and self.s_shift is None:
            try:
                lambda_max = eigsh(L_uu, k=1, which='LM', return_eigenvectors=False)[0]
                s = lambda_max * 1.01  # Small buffer for numerical stability
            except:
                s = 1.0  # Fallback
        else:
            s = self.s_shift if self.s_shift is not None else 1.0
        
        # Compute M = s*I - L_uu (PSD matrix)
        M = s * sparse.eye(n_unlabeled) - L_uu
        
        # Initialize X_u using specified method
        X_u = self._initialize_unlabeled_assignments(n_unlabeled, k, m_u, L_uu, B, train_ind, train_labels)
        
        # MM iterations
        for t in range(self.max_iter):
            # Compute current objective value
            F_current = self._compute_objective(X_u, M, B)
            
            # Compute C^(t) = M @ X^(t) + B
            C_t = M @ X_u + B
            
            # Solve surrogate problem using projection method
            X_u_new = self._solve_surrogate_projection(C_t, m_u, X_u)
            
            # Check convergence
            F_new = self._compute_objective(X_u_new, M, B)
            rel_change = abs(F_new - F_current) / max(abs(F_current), 1e-10)
        ...
        X_final = np.zeros((n, k))
        X_final[train_ind] = Y           # Labeled nodes
        X_final[unlabeled_mask] = X_u    # Unlabeled nodes
        
        # Compute accuracy if ground truth available
        if all_labels is not None:
            pred = X_final.argmax(axis=1)
            acc = ssl_accuracy(pred, all_labels, train_ind)
...
from scipy.sparse.linalg import eigsh
import graphlearning as gl  # Assuming this is the library providing ssl base class and utils

class stiefel_ssl(ssl):
    def __init__(self, W=None, class_priors=None, maxiter=1, tol=1e-5, beta=0.5, c=1e-4):
        """StiefelSSL
        ===================
        Semi-supervised learning via Laplace learning on Stiefel manifolds (with C = I approximation)
        ...
        W : numpy array, scipy sparse matrix, or graphlearning graph object (optional), default=None
            Weight matrix representing the graph.
        class_priors : numpy array (optional), default=None
            Class priors (fraction of data belonging to each class). Not used since C = I.
        maxiter : int (optional), default=100
            Maximum number of iterations for gradient projection.
        tol : float (optional), default=1e-5
            Tolerance for convergence based on Riemannian gradient norm.
        ...
        self.beta = beta
        self.c = c
        self.name = 'StiefelSSL'
        self.accuracy_filename = '_stiefelssl'
        self.k = None  # Will be set in _fit

    def _objective(self, X, L, B):
        """Compute the objective function value."""
        LX = L @ X
        return 0.5 * np.trace(X.T @ LX) + 0.5 * np.linalg.norm(X - B, 'fro')**2

    def _eucl_grad(self, X, L, B):
        """Compute the Euclidean gradient."""
        return L @ X + X - B

    def _riem_grad_norm(self, X, eucl_grad):
        """Compute the norm of the Riemannian gradient."""
        sym = 0.5 * (X.T @ eucl_grad + eucl_grad.T @ X)
        grad_tan = eucl_grad - X @ sym
        return np.linalg.norm(grad_tan, 'fro')

    def _project_stiefel(self, Y):
        """Project onto the Stiefel manifold via SVD."""
    ...
    def _fit(self, train_ind, train_labels, all_labels=None):
        ...
        G = gl.graph(self.graph.weight_matrix)
        L = G.laplacian(normalization='combinatorial')  # Assuming combinatorial Laplacian
        ...
        # Label matrix B (n x k)
        B = np.zeros((n, self.k))
        F = gl.utils.labels_to_onehot(train_labels, self.k)
        B[train_ind, :] = F

        # Initial approximation via eigenvectors + Procrustes
        _, evecs = eigsh(L, k=self.k, which='SM')  # Smallest k eigenvalues/vectors
        M = evecs.T @ B
        U, S, Vt = np.linalg.svd(M, full_matrices=False)
        Q = U @ Vt
        X = evecs @ Q

        # Precompute L + I for gradient, but since grad = (L + I) X - B
        I = np.eye(n) if not sparse.issparse(L) else sparse.eye(n, format='csr')
        L_plus_I = L + I

        # Iterative gradient projection
        for i in range(self.maxiter):
            eucl_grad = L_plus_I @ X - B
            riem_norm = self._riem_grad_norm(X, eucl_grad)
            if riem_norm < self.tol:
                break

            f_current = self._objective(X, L, B)

            # Armijo line search
            alpha = 1.0
            while True:
                X_trial = X - alpha * eucl_grad
                X_new = self._project_stiefel(X_trial)
                f_new = self._objective(X_new, L, B)
                dir_deriv = np.trace(eucl_grad.T @ (X_new - X))
                if f_new <= f_current + self.c * dir_deriv:
                    X = X_new
                    break
                alpha *= self.beta
                if alpha < 1e-10:
            ...
            # Optional accuracy reporting
            if (all_labels is not None) and (i % 25 == 0):
                acc = gl.ssl.ssl_accuracy(X.argmax(1), all_labels, train_ind)
                print(f'Iteration {i}, Accuracy = {acc:.2f}')
